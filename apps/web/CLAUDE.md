...
- Tailwind CSS v4 + HeroUI v3 (beta)
...
- HeroUI v3 は **compound components**（`Card.Header`, `Card.Content` など）。v2 とは API が異なるので、コンポーネント追加時は HeroUI MCP / 公式 docs で v3 を確認すること。
- Tailwind は v4。`postcss.config.mjs` + `@tailwindcss/postcss` 構成。
